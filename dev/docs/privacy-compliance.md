...
The full check also passed static export tests, server tests, and the Cloudflare Worker build. Its bundle check caught duplicate Klaro CSS; the base stylesheet was moved out of public assets into the application styles so only the bundled copy ships. Browser preflight remains blocked by unavailable Chromium.
