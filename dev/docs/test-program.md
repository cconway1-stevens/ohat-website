...
| **Lighthouse**             | Perf/a11y/BP/SEO per page                                        | lighthouse + Playwright Chromium | `check-lighthouse.mjs`, AD-3/AD-6 | `check:lighthouse:fast` or `check:lighthouse` | lighthouse matrix (fast), resilience (3-run median) | **25 indexable routes by default** | Yes (auto-builds if missing) | dist/client        | deterministic thresholds; performance advisory | Yes for a11y/BP/SEO | Four stable CI shards; noindex routes are opt-in  |
