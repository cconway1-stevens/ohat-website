...
export function NightDrive() {
  ...
  const canvasRef = useSceneCanvas((ctx, frame) => {
    ...
    if (weather === "clear") {
      ...
      ctx.beginPath(); ctx.arc(392, 30, 13, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#0a0d18";
      ctx.beginPath(); ctx.arc(386, 27, 12, 0, Math.PI * 2); ctx.fill();
    }
    ...
    if (pass < 0.42) {
      ...
      ctx.beginPath(); ctx.arc(W / 2 - 30 - spread, y, 2 + t * 6, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(W / 2 - 8 - spread, y, 2 + t * 6, 0, Math.PI * 2); ctx.fill();
    }
    ...
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.arc(78, H - 22, 15, Math.PI * 0.8, Math.PI * 2.2); ctx.stroke();
    ctx.beginPath(); ctx.arc(W - 78, H - 22, 15, Math.PI * 0.8, Math.PI * 2.2); ctx.stroke();
    ctx.fillStyle = "rgba(246,189,56,.85)";
  ...
  return (
    ...
    >
      <div className="cozy-stage">
        <canvas ref={canvasRef} width={W} height={H} role="img" aria-label="Driving a dark road at night, seen over the dashboard" />
      </div>
      ...
      <div className="cozy-actions">
        <button
          ...
          onClick={() => {
            ...
            setHeater(next);
            setNote(next === 0 ? "Heater off. The glass starts to mist." : next === 1 ? "Heater on low. Just enough." : "Heater on high. Toes are happy.");
          }}
        >
        ...
        <button
          ...
          aria-pressed={wipers}
          onClick={() => { cozyAudio.wiper(); setWipers((on) => !on); setNote(wipers ? "Wipers off." : "Wipers on. Steady rhythm."); }}
        >
