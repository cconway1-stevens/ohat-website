...
export function SundayWash() {
  ...
  const canvasRef = useSceneCanvas((ctx, frame) => {
    ...
    ctx.beginPath(); ctx.arc(408, 44, 22, 0, Math.PI * 2); ctx.fill();
    ...
    for (const wx of [baseX + 34, baseX + len - 40]) {
      ...
      ctx.beginPath(); ctx.arc(wx, baseY + 2, 17, 0, Math.PI * 2); ctx.fill();
      // Tire shine is the last pass, and it shows.
      ctx.fillStyle = step > 4 ? "#4b4741" : "#2e2b27";
      ctx.beginPath(); ctx.arc(wx, baseY + 2, 13, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#cfc9b8";
      ctx.beginPath(); ctx.arc(wx, baseY + 2, 6, 0, Math.PI * 2); ctx.fill();
    }
  ...
  function nextCar() {
    ...
    setStep(0);
    setNote(`Here comes ${VEHICLES[(vehicle + 1) % VEHICLES.length].name}, still wearing the week.`);
  }
  ...
  return (
    ...
    >
      <div className="cozy-stage">
        <canvas ref={canvasRef} width={W} height={H} role="img" aria-label={`A wash bay with ${car.name} part way through a wash`} />
      </div>
