...
export function ClosingTime() {
  ...
  const canvasRef = useSceneCanvas((ctx, frame) => {
    ...
    for (let cart = 0; cart < 2; cart += 1) {
      ...
      ctx.beginPath(); ctx.arc(x + 7, y + 28, 3, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.arc(x + 27, y + 28, 3, 0, Math.PI * 2); ctx.fill();
    }
  ...
  return (
    ...
    >
      <div className="cozy-stage">
        <canvas ref={canvasRef} width={W} height={H} role="img" aria-label="A quiet garage at night with three service bays, tool carts and rain outside the open door" />
      </div>
      ...
      <div className="cozy-actions">
        ...
        <button
          type="button"
          disabled={carts >= 2}
          onClick={() => { cozyAudio.drawer(); setCarts((c) => c + 1); setLast("A cart rolls back against the wall."); }}
        >
        ...
        <button
          ...
          onClick={() => { cozyAudio.drawer(); setDoorDown((d) => !d); setLast(doorDown ? "The door rolls back up. Cool air and rain." : "The door comes down. The rain goes quiet."); }}
        >
          {doorDown ? "Open the door" : "Pull the door down"}
        </button>
        <button
          type="button"
          onClick={() => { cozyAudio.compressor(); setLast("The compressor tops itself up, then cuts out."); }}
        >
