...
export function GarageRadio() {
  ...
  const [note, setNote] = useState(
    "Somebody's car is up on the lift out there. You have a chair and a coffee machine.",
  );
  ...
  const canvasRef = useSceneCanvas(
    (ctx, frame) => {
      ...
      ctx.beginPath();
      ctx.arc(walk + 6, 106, 6, 0, Math.PI * 2);
      ctx.fill();
    ...
    >
      <div className="cozy-stage">
        <canvas
          ...
          height={H}
          role="img"
          aria-label="A garage waiting room looking through glass at a car on a lift"
        />
      </div>
      ...
      <div className="cozy-actions">
        ...
        <button
          type="button"
          onClick={tap(
            "Two quarters go in and something lands at the bottom with a thud.",
            cozyAudio.coin,
          )}
        >
          Vending machine
        ...
        <button
          type="button"
          onClick={tap(
            "A drawer of sockets rolls open somewhere out in the bay.",
            cozyAudio.drawer,
          )}
        >
          Tool drawer
        ...
        <button
          ...
          onClick={() => {
            ...
            setNote(
              raining
                ? "The rain lets up. Somebody props the side door open."
                : "It starts up again against the glass.",
            );
          }}
        >
        ...
        <button
          type="button"
          onClick={tap(
            "The bell over the door goes as somebody else comes in to wait.",
            cozyAudio.doorBell,
          )}
        >
          Door bell
      ...
      <p className="cozy-note">
        {coffees === 0
          ...
          : `${coffees} free coffee${coffees === 1 ? "" : "s"} in. They should be done soon.`}
      </p>
    </CozyShell>
  );
}
