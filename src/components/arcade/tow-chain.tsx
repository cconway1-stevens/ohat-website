...
// The status line for the service call currently running. Missing this once
// meant the render threw the moment a bonus was picked up — keep it next to
// the kinds it describes so the two never drift apart again.
const BONUS_LABEL: Record<BonusKind, string> = {
  ghost: "Ghost run — the rig passes through its own chain.",
  slow: "Easy does it — half speed for a spell.",
  trim: "Quick drop-off — the chain is shorter.",
};
...
export function TowChain() {
  ...
  useEffect(() => {
    ...
    // A timeout chain rather than one fixed interval: every car on the hook
    // shaves the pace a touch (classic snake ramp), floored so a long chain
    // stays drivable. The slow service call still halves the pace inside
    // tick() itself, on top of whatever speed the chain has reached.
    let timer = 0;
    const loop = () => {
      // A hidden tab throttles timers to about one a second, which reads as
      // the truck teleporting into the fence. Hold position instead — the
      // shift resumes exactly where it was when the tab comes back.
      if (document.hidden) {
      ...
      timer = window.setTimeout(loop, Math.max(96, TICK_MS - s.towed * 6));
    };
