...
export function ServiceSearch() {
  ...
  // Drag selection: the press only arms once the pointer actually travels,
  // so plain taps keep working through the click handler exactly as before.
  // `trail` is the live elastic-band preview while a drag is underway.
  ...
  /** The cell under a pointer, found by hit-testing: touch pointers stay
      captured by the cell they started on, so event.target never moves. */
  function cellFromPoint(event: React.PointerEvent): Point | null {
    const element = document.elementFromPoint(event.clientX, event.clientY);
    const button = element?.closest("button[data-row]");
    if (!button) return null;
    const { row, col } = (button as HTMLElement).dataset;
  ...
  function onGridPointerMove(event: React.PointerEvent) {
    ...
    if (!from || !puzzle || complete) return;
    if (!dragging.current) {
      dragging.current = true;
      setStart(from);
      setMessage("Drag to the last letter, then let go.");
      if (sound) garageAudio.beep(360);
    }
    const hover = cellFromPoint(event);
    setTrail(hover ? lineBetween(from, hover) : []);
  }

  function onGridPointerUp(event: React.PointerEvent) {
    const from = dragFrom.current;
    dragFrom.current = null;
    setTrail([]);
    const wasDragging = dragging.current;
    dragging.current = false;
    if (!from || !wasDragging || !puzzle || complete) return;
    const up = cellFromPoint(event);
    // Off the board: the first letter stays lit, so the word can still be
    // finished with a plain tap on the last letter.
    if (!up) return;
    if (up.row === from.row && up.col === from.col) {
      // Out and back to the first letter: call it a cancelled drag.
      setStart(null);
      setMessage("Tap the first letter, then the last letter. Skip the letters in between.");
      return;
    }
    chooseCell(up.row, up.col);
  }
            ...
            letters.map((letter, col) => {
              ...
              return (
                <button
                  ...
                  data-row={row}
                  data-col={col}
                  className={`${foundCells.has(key) ? "is-found" : revealedCells.has(key) ? "is-revealed" : ""}${selected ? " is-selected" : ""}${trail.includes(key) ? " is-trail" : ""}`}
