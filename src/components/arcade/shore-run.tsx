...
// On phone-sized screens the logical width narrows from 640 to 420: every
// spawn, gap and speed is measured in logical pixels, so the game plays
// identically — the CSS just scales each of those pixels ~50% larger, which
// is what makes the car readable on a 360px display.
const W_DESKTOP = 640;
const W_NARROW = 420;
let W = W_DESKTOP;
const H = 200;
...
export function ShoreRun() {
  ...
  // The canvas attribute tracks the logical width, narrowed on phones (see
  // the note by W_DESKTOP). Read once at mount; rotating mid-run just keeps
  // the width the run started with.
  const [viewW] = useState(() => {
